...
// todo - add test cases for logging in with email address.
	// probably needs to be in every single one.
	// probably need to refactor so it's DRY

use \Codeception\Scenario;

class Google_Authenticator_Per_User_Prompt_Acceptance_Tests_Cest {
	...
	public function use_plugin_without_google_authenticator( WebGuy $i, Scenario $scenario ) {
		$i->wantTo( 'Prevent using the plugin when Google Authenticator is not active.' );
	...
	public function login_with_invalid_username_or_password( WebGuy $i, Scenario $scenario ) {
		$i->wantTo( 'Prevent logging in with an invalid username or password.' );
	...
	public function login_with_expired_nonce( WebGuy $i, Scenario $scenario ) {
		$i->wantTo( 'Prevent logging in with an expired nonce.' );
		...
		$i->sendOtp(
			GAPUPAT_One_Time_Passwords::getCurrentOtp( self::OTP_SECRET, self::OTP_LIFETIME_SECONDS )
		);
	...
	public function login_with_expired_otp( WebGuy $i, Scenario $scenario ) {
		$i->wantTo( 'Prevent logging in with an expired OTP.' );
	...
	public function login_with_invalid_otp( WebGuy $i, Scenario $scenario ) {
		$i->wantTo( 'Prevent logging in with an invalid OTP.' );
	...
	public function visit_2fa_form_directly_invalid_nonce( WebGuy $i, Scenario $scenario ) {
		$i->wantTo( 'Prevent bypassing the username/password form with an invalid nonce.' );
	...
	public function visit_2fa_form_directly_missing_nonce( WebGuy $i, Scenario $scenario ) {
		$i->wantTo( 'Prevent bypassing the username/password form with an empty nonce.' );
	 ...
	 * @param Scenario $scenario
	 */
	public function login_to_xmlrpc_with_valid_application_password( WebGuy $i, Scenario $scenario ) {
		$i->wantTo( 'Login to XML-RPC with a valid application password.' );
	 ...
	 * @param Scenario $scenario
	 */
	public function login_to_xmlrpc_with_invalid_application_password( WebGuy $i, Scenario $scenario ) {
		$i->wantTo( 'Prevent logging in to XML-RPC with an invalid application password.' );
	 ...
	 * @param Scenario $scenario
	 */
	public function login_to_xmlrpc_when_application_passwords_disabled( WebGuy $i, Scenario $scenario ) {
		$i->wantTo( 'Prevent logging in to XML-RPC when application passwords are disabled.' );
	 ...
	 * @param Scenario $scenario
	 */
	public function login_to_wp_admin_with_valid_application_password( WebGuy $i, Scenario $scenario ) {
		$i->wantTo( 'Prevent logging in to wp-admin with a valid application password.' );
	 ...
	 * @param Scenario $scenario
	 */
	public function login_to_wp_admin_with_invalid_application_password( WebGuy $i, Scenario $scenario ) {
		$i->wantTo( 'Prevent logging in to wp-admin with an invalid application password.' );
